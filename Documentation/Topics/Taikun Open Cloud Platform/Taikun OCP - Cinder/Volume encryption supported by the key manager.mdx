...
1. Restart `cinder-api`, `cinder-volume` and `cinder-backup`.
...
> NoteUse a ‘#’ prefix to comment out the line in this section that begins with ‘fixed\_key’.
...
1. Create an unencrypted 1GB test volume:

`$ openstack volume create --size 1 'unencrypted volume'`

1. Create an encrypted 1GB test volume:
...
> NoteThe `--device` option to specify the mountpoint for the attached volume may not be where the block device is actually attached in the guest VM, it is used here for illustration purposes.
