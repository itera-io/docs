...
1. Check the inventory of each resource provider to see resource classes
...
1. Check allocations for each server that is using virtual GPUs
...
1. Define custom traits that will correspond for each to a GPU type
...
1. Add the corresponding trait to the Resource Provider matching the GPU
...
1. Amend the flavor to add a requested trait
