...
1. To list the loaded kernel modules and verify that the `kvm` modules are loaded, run this command:
...
> > NoteSome systems require that you enable VT support in the system BIOS. If you believe your processor supports hardware acceleration but the previous command did not produce output, reboot your machine, enter the system BIOS, and enable the VT option.
...
1. To list the loaded kernel modules and verify that the `kvm` modules are loaded, run the following command:
