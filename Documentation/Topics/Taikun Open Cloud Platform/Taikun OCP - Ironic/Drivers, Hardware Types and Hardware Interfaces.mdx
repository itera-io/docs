...
1. Request resetting some of the interfaces to their new defaults by using the `--reset-<IFACE>-interface` family of arguments, for example:
...
> NoteThis feature is available starting with ironic 11.1.0 (Rocky series, API version 1.45).

1. Request resetting all interfaces to their new defaults:
...
1. Change the node to use the `noop` management interface:
