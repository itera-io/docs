...
InterfaceSupported Implementations
...
`idrac-wsman`, `idrac`, `idrac-redfish`, `no-raid`
...
> Note`idrac` is the legacy name of the WSMAN interface. It has been deprecated in favor of `idrac-wsman` and may be removed in a future release.
...
> NoteIf using WSMAN for the management interface, then WSMAN must be used for the power interface. The same applies to Redfish. It is currently not possible to use Redfish for one and WSMAN for the other.
...
> NoteiDRAC BMC connection settings are not exported to avoid overwriting these in another system when using unmodified exported configuration mold in import step. If need to replicate iDRAC BMC connection settings, then add these settings manually to configuration mold for import step.
...
> NoteUse of TLS is strongly advised.
...
> NoteWhen using `idrac-redfish` for RAID interface iDRAC firmware greater than 4.40.00.00 is required.
...
> Note`JBOD` and `2` are not supported, and will fail with reason: ‘Cannot calculate spans for RAID level.’
...
> Note`physical_disks` is a mandatory parameter if the property `size_gb` is set to `MAX`.
...
Method NameHTTP MethodDescription
