...
1. Generate the SSL certificate, here is an example, you can find more about openssl on the [openssl page](https://www.openssl.org/):
...
* Configure node web console.
  ...
  If the node uses a hardware type, for example `ipmi`, set the node’s console interface to `ipmitool-shellinabox`:
...
> NoteAn error message you may encounter when enabling the console can read `Console subprocess failed to start. Timeout or error while waiting for console subprocess to start for node` along with `[server] Failed to find any available port!`. This error is coming from shellinabox itself, not from the communication with the BMC. One potential cause for this issue is that there are already shellinabox daemons running which block the configured port (remove them if appropriate and retry to enable the console).
...
* Configure node console.
  ...
  If the node uses a hardware type, for example `ipmi`, set the node’s console interface to `ipmitool-socat`:
