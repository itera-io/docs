...
* To temporarily disable a user account:
  ...
  ```
  $ openstack user set USER\_NAME --disableIf
  ```
  you disable a user account, the user cannot log in to the dashboard. However, data for the user account is maintained, so you can enable the user at any time.
...
* To change the name and description for a user account:
  ...
  ```
  $ openstack user set USER\_NAME --name user-new --email
  ```
