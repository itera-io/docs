...
* To change the name and description for a user account:\`\`\`
  $ openstack user set USER\_NAME --name user-new --email```User has been updated.
  ```
...
1. Assign a role to a user-project pair:`$ openstack role add --user USER\_NAME --project PROJECT\_NAME ROLE\_NAME`For example, assign the `new-role` role to the `demo` user and `test-project` project pair:\`\`\`
